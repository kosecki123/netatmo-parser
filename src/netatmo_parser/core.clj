...
(defn read-config []
    (read-string (slurp (clojure.java.io/file "resources/config.edn")))
    )
...
(defn get-token []
  (let [
         query {:form-params (read-config) :insecure? true}
...
(defn read-netatmo-file []
  (slurp (clojure.java.io/file "resources/netatmo.ger.json")))

(defn deserialize []
  (json/read-json (read-netatmo-file))
  )
...
(defn get-temperature [{:keys [type res]}]
  (let [
         value-index (index-of "temperature" type)
...
(defn get-location-from-body [{:keys [results]}]
  (let [
         area (get-in (first results) [:geometry :bounds])
         ne (:northeast area)
         ne_lat (get ne :lat)
         ne_lon (get ne :lng)
         sw (:southwest area)
